...
The module starts /app/server.py as a subprocess, waits until the gRPC
port is accepting connections, then exercises all three RPCs over real
network calls on localhost:50051.
...
# grpcio/protobuf are installed at the system Python's site-packages by the
# Dockerfile.  Add that path so they are importable inside the uvx venv that
# runs pytest, avoiding any runtime package installation.
_SITE = "/usr/local/lib/python3.12/site-packages"
if _SITE not in sys.path:
    sys.path.insert(0, _SITE)

# Add /app so the generated stubs and server module are importable.
...
try:
    grpc.channel_ready_future(_channel).result(timeout=30)
...
stub = inventory_pb2_grpc.InventoryServiceStub(_channel)
...
def test_check_inventory_laptop():
...
def test_list_products_stock_values():
...
# ===========================================================================
# ReserveInventory — ordered mutation tests (each uses a distinct product)
# ===========================================================================
...
def test_reserve_over_stock_raises_resource_exhausted():
...
def test_reserve_zero_stock_raises_resource_exhausted():
...
def test_sequential_reservations_accumulate_correctly():
